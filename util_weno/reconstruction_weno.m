...
%   eps = 1e-10;
  if k==2
    alphar = [0.666666666666667; 0.333333333333333]./ (eps + [ ( v(end,:)-v(end-1,:) ).^2.0;...
                                                                 ( v(end-1,:)-v(1,:)   ).^2.0 ]);%.^p;
    wr = alphar ./ sum(alphar);
    v_weno = wr(1,:) .* sum([0.5;0.5].*v(k:end,:))+ ...
             wr(2,:) .* sum([-0.5;1.5].*v(k-1:end-1,:));
  elseif k==3
    alphar = [0.3; 0.6; 0.1]./ (eps + [ 1.083333333333333*( v(end-2,:)-2.0*v(end-1,:)+v(end,:) ).^2.0+0.25*( 3.0*v(end-2,:)-4.0*v(end-1,:)+v(end,:) ).^2.0;
                                          1.083333333333333*( v(end-3,:)-2.0*v(end-2,:)+v(end-1,:) ).^2.0+.25*( v(end-3,:)-v(end-1,:) ).^2.0;
                                          1.083333333333333*( v(end-4,:)-2.0*v(end-3,:)+v(end-2,:) ).^2.0+0.25*( v(end-4,:)-4.0*v(end-3,:)+3.0*v(end-2,:) ).^2.0 ]);%.^p;
    wr = alphar ./ sum(alphar);
  ...
  elseif k==4
    alphar = [0.028571428571429; 0.342857142857143; 0.514285714285714; 0.114285714285714]./ (eps + [ v(end-3,:).*( 2107.0*v(end-3,:)-9402.0*v(end-2,:)+7042.0*v(end-1,:)-1854.0*v(end,:) )+v(end-2,:).*( 11003.0*v(end-2,:)-17246.0*v(end-1,:)+4642.0*v(end,:) )+v(end-1,:).*( 7043.0*v(end-1,:)-3882.0*v(end,:) )+547.0*v(end,:).^2.0;
                                                                                                       ...
                                                                                                       v(end-6,:).*( 547.0*v(end-6,:)-3882.0*v(end-5,:)+4642.0*v(end-4,:)-1854.0*v(end-3,:) )+v(end-5,:).*( 7043.0*v(end-5,:)-17246.0*v(end-4,:)+7042.0*v(end-3,:) )+v(end-4,:).*(11003.0*v(end-4,:)-9402.0*v(end-3,:) )+2107.0*v(end-3,:).^2.0 ]);%.^p;
    wr = alphar ./ sum(alphar);
  ...
  elseif k==5
    alphar = [0.007936507936508; 0.158730158730159;0.476190476190476;0.317460317460317;0.039682539682540]./ (eps + [ v(end-4,:).*( 107918.0*v(end-4,:)-649501.0*v(end-3,:)+758823.0*v(end-2,:)-411487.0*v(end-1,:)+86329.0*v(end,:) )+v(end-3,:).*( 1020563.0*v(end-3,:)-2462076.0*v(end-2,:)+1358458.0*v(end-1,:)-288007.0*v(end,:) )+v(end-2,:).*( 1521393.0*v(end-2,:)-1704396.0*v(end-1,:)+364863.0*v(end,:) )+v(end-1,:).*( 482963.0*v(end-1,:)-208501.0*v(end,:) )+22658.0*v(end,:).^2.0;
                                                                                                                       ...
                                                                                                                       v(end-8,:).*( 22658.0*v(end-8,:)-208501.0*v(end-7,:)+364863.0*v(end-6,:)-288007.0*v(end-5,:)+86329.0*v(end-4,:) )+v(end-7,:).*( 482963.0*v(end-7,:)-1704396.0*v(end-6,:)+1358458.0*v(end-5,:)-411487.0*v(end-4,:) )+v(end-6,:).*( 1521393.0*v(end-6,:)-2462076.0*v(end-5,:)+758823.0*v(end-4,:) )+v(end-5,:).*(1020563.0*v(end-5,:)-649501.0*v(end-4,:) )+107918.0*v(end-4,:).^2.0 ]);%.^p;
    wr = alphar ./ sum(alphar);
  ...
  elseif k==6
    alphar = [0.002164502164502;0.064935064935065;0.324675324675325;0.432900432900433;0.162337662337662;0.012987012987013]./ (eps + [ v(end-5,:).*( 6150211.0*v(end-5,:)-47460464.0*v(end-4,:)+76206736.0*v(end-3,:)-63394124.0*v(end-2,:)+27060170.0*v(end-1,:)-4712740.0*v(end,:) )+v(end-4,:).*( 94851237.0*v(end-4,:)-311771244.0*v(end-3,:)+262901672.0*v(end-2,:)-113206788.0*v(end-1,:)+19834350.0*v(end,:) )+v(end-3,:).*( 260445372.0*v(end-3,:)-444003904.0*v(end-2,:)+192596472.0*v(end-1,:)-33918804.0*v(end,:) )+v(end-2,:).*( 190757572.0*v(end-2,:)-166461044.0*v(end-1,:)+29442256.0*v(end,:) )+v(end-1,:).*( 36480687.0*v(end-1,:)-12950184.0*v(end,:) )+1152561.0*v(end,:).^2.0;
