%% Explicit Bogacki-Shampine method of order 3 (ode23)
function X = ebs23(tspan,X0,dt)
global Nzm
...
%       yn(2*Nzm+(1:Nzm)) = ImDiffMF(yn(2*Nzm+(1:Nzm)),dt/2); %Symm. Splitting only minor differences
      % Explicit terms
      k1 = expFun(t,yn);
      k2 = expFun(t,yn+0.5*dt*k1);
      k3 = expFun(t,yn+0.75*dt*k2);
      ynp1 = yn + dt*(k1*2/9 + k2/3 + k3*4/9);
      k4 = expFun(t,ynp1);
      yn = yn + dt*(k1*7/24 + k2/4 + k3/4 + k4/8);
      % Nonnegative
