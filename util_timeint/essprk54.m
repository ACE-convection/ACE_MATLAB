%% Explicit 5-stage 4th-order Strong Stability Preserving Runge-Kutta (eSSPRK54)
function X = esppk54(tspan,X0,dt)
global Nzm
...
%       yn(2*Nzm+(1:Nzm)) = ImDiffMF(yn(2*Nzm+(1:Nzm)),dt/2); %Symm. Splitting only minor differences
      % Explicit terms
      k1 = yn + (0.391752226571890*dt)*odeFun(t,yn);
      k2 = 0.444370493651235*yn + 0.555629506348765*k1 + (0.368410593050371*dt)*odeFun(t,k1);
      k3 = 0.620101851488403*yn + 0.379898148511597*k2 + (0.251891774271694*dt)*odeFun(t,k2);
      Fk3 = odeFun(t,k3);
      ...
      yn = 0.517231671970585*k2 + 0.096059710526147*k3 + (0.063692468666290*dt)*Fk3 +...
           0.386708617503268*k4 + (0.226007483236906*dt)*odeFun(t,k4);
      % Nonnegative
      yn(Nzm+(1:Nzm)) = yn(Nzm+(1:Nzm)).*(yn(Nzm+(1:Nzm))>0);
      % Implicitly solving massflux diffusion (repeated for symmetric splitting)
      yn(2*Nzm+(1:Nzm)) = ImDiffMF(yn(2*Nzm+(1:Nzm)),dt); %dt/2 for symmetric splitting
    end
    X(:,idt+1) = yn;
  end
