%% 4-stage 3rd-order Strong Stability Preserving Runge-Kutta (SSPRK3)
function X = ssprk3(tspan,X0,dt)
global Nzm
...
%       yn(2*Nzm+(1:Nzm)) = ImDiffMF(yn(2*Nzm+(1:Nzm)),dt/2); %Symm. Splitting only minor differences
      % Explicit terms
      k1 = yn + (dt/2)*odeFun(t,yn);
      k2 = k1 + (dt/2)*odeFun(t,k1);
      k3 = (2/3)*yn + k2/3 + (dt/6)*odeFun(t,k2);
      yn = k3 + (dt/2)*odeFun(t,k3);
      % Nonnegative
